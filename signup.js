  ...
  var app = angular.module('sbaapp', ['ngResource', 'ngFileUpload']); 
        ...
        app.controller('loginController', [ '$scope', '$resource', '$window', function($scope, $resource, $window) {
             $scope.master = {username: '', password: '',email: '', };  
               $scope.sbaProj = {};
            ...
            $scope.anonymouslogin = function () { 
              ...
              User.get( { identifier: $scope.user.identifier }, function(data){
            ...
            $scope.resetpass = function () { 
              ...
              var User = $resource('http://strabbodevapi-211215818.us-east-1.elb.amazonaws.com/standbyapps-server/api/unauth/resetPassword', {
               password:'@password', reset_token : '@reset_token' }); 

              $scope.user.reset_token = ''; //get token from url -- GET argument token__ 
              User.get( {password: $scope.user.password, reset_token : $scope.user.reset_token }, function(data){
            ...
            $scope.forgotpass = function () { 
              ...
              User.get( {username: $scope.user.username }, function(data){
            ...
            $scope.login = function(cdata) { 
              ...
              var lvarst = { username:'@username' , password: '@password'};  
               var lvars =  {username: $scope.user.username, password: $scope.user.password }; 
               
              //check if facebook token present __ then use that instead of user/pass _ 
              if( cdata != undefined && cdata.facebook_token != undefined ) { 
                lvarst = {facebook_token : '@facebook_token', options : "FLAG_HOST_ROLE" }; 
                lvars = {facebook_token : cdata.facebook_token,  options : "FLAG_HOST_ROLE"};  
              } 
              if( cdata != undefined && cdata.google_access_token != undefined && $window.sessionStorage.session_token == undefined ) { 
                lvarst = {google_access_token : '@google_access_token',  options : "FLAG_HOST_ROLE"}; 
                lvars = {google_access_token : cdata.google_access_token,  options : "FLAG_HOST_ROLE"};  
              } 
   ...
   app.controller('registerController', [ '$scope', '$resource', 'Upload', '$timeout',   function($scope, $resource, Upload, $timeout) { 
          $scope.master = {first_name: '', last_name: '', password : '',  email : '' ,  username : '', phone : '', dob : '', image_url : '', options: '' };   
          $scope.pmaster = {name: '', description: '', address_street_1 : '',  address_street_2 : '' ,  city : '', state : '', country : '', zip_code : ''};   
          ...
          $scope.getHostProperty = function () { 
             ...
             var ApiRes = $resource('http://strabbodevapi-211215818.us-east-1.elb.amazonaws.com/standbyapps-server/api/property/myProperties', {} , {get: { 'method': 'GET', headers: { 'Content-Type': 'application/x-www-form-urlencoded' , 'Authorization': '' + window.sessionStorage.session_token } }} ); 
          ...
          $scope.addPropertyImg2 = function (file, errFiles , e_pic_id) {  
             ...
             if(window.sessionStorage.last_propert_unique_id == undefined) { 
              //property not added yet... try that first __ 
                ...
                setTimeout(function(){ $scope.addPropertyImg2(file, errFiles , e_pic_id);  }, 5000);
           ...
           if (file) { 
            ...
            file.upload = Upload.upload({
                url: rurl,
                data: {photo: file},  //, 'property_id' : $scope.property.property_id  
                headers: {'Authorization': window.sessionStorage.session_token},  
          ...
          $scope.addProperty = function () { 
              ...
              var ApiRes = $resource('http://strabbodevapi-211215818.us-east-1.elb.amazonaws.com/standbyapps-server/api/property/create',{}, {save: { 'method': 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' , 'Authorization': '' + window.sessionStorage.session_token } }} ); 
            ...
            $scope.getProfile = function () { 
              ...
              var User = $resource('http://strabbodevapi-211215818.us-east-1.elb.amazonaws.com/standbyapps-server/api/user/profile', {  }, {get: { 'method': 'GET', headers: {  'Content-Type': 'application/x-www-form-urlencoded' , 'Authorization': '' + window.sessionStorage.session_token } }} );
            ...
            $scope.confirm_email = function () { 
              ...
              var User = $resource('http://strabbodevapi-211215818.us-east-1.elb.amazonaws.com/standbyapps-server/api/user/verifyEmail', {
               email:'@email' }, {get: { 'method': 'GET', headers: {  'Content-Type': 'application/x-www-form-urlencoded' , 'Authorization': '' + window.sessionStorage.session_token } }} );
              
              User.get( {email: $scope.user.email }, function(data){
            ...
            $scope.confirm_phone = function () { 
              ...
              var User = $resource('http://strabbodevapi-211215818.us-east-1.elb.amazonaws.com/standbyapps-server/api/user/verifyPhone', {
               phone:'@phone' }, {get: { 'method': 'GET', headers: {  'Content-Type': 'application/x-www-form-urlencoded' , 'Authorization': '' + window.sessionStorage.session_token } }} );
              
              User.get( {phone: $scope.user.phone }, function(data){
